// Holland Code Career Interest Questions (RIASEC Model)
// Group A: Class 8th-10th (30 questions) - Interests and Hobbies
// Group B: Class 11th-12th (50 questions) - Stream Alignment
// Group C: College & Graduates (75 questions) - Professional Fit
...
export interface HollandQuestion {
  ...
  classMin: number;
  classMax: number;
  clusterWeights?: Record<string, number>; // For CLUSTER assessment type
  branchWeights?: Record<string, number>; // For BRANCH assessment type
}

// Group A: Class 8th - 10th (30 Questions) - Interests and Hobbies
export const GROUP_A_QUESTIONS: HollandQuestion[] = [
...
export const GROUP_B_QUESTIONS: HollandQuestion[] = [
...
export const GROUP_C_QUESTIONS: HollandQuestion[] = [
...
export const HOLLAND_CODE_QUESTIONS = [
  ...GROUP_A_QUESTIONS,
  ...GROUP_B_QUESTIONS,
  ...GROUP_C_QUESTIONS,
];
...
export function getQuestionsForClassLevel(classLevel: number): HollandQuestion[] {
...
export function getQuestionsByType(type: "R" | "I" | "A" | "S" | "E" | "C"): HollandQuestion[] {
  return HOLLAND_CODE_QUESTIONS.filter((q) => q.type === type);
}
```

Now let me create the Holland Code types definition file:

```typescript
/**
 * Holland Code (RIASEC) Types and Career Mapping
 *
 * R - Realistic: Practical, hands-on, mechanical, outdoor work
 ...
 * E - Enterprising: Leadership, business, influencing others
 * C - Conventional: Organized, detail-oriented, structured tasks
 */

export type HollandCodeType = "R" | "I" | "A" | "S" | "E" | "C";

export interface HollandCodeDescription {
  code: HollandCodeType;
  title: string;
  description: string;
  keywords: string[];
...
export const HOLLAND_CODES: Record<HollandCodeType, HollandCodeDescription> = {
  R: {
    code: "R",
    title: "Realistic",
...
export const CLUSTER_HOLLAND_CODE_MAPPING: Record<string, HollandCodeType[]> = {
  "Engineering": ["R", "I"],
...
// Career Branches mapped to Holland Codes
export const BRANCH_HOLLAND_CODE_MAPPING: Record<string, HollandCodeType[]> = {
  // Engineering branches
  "Computer Science": ["I", "C"],
  "Mechanical Engineering": ["R", "I"],
  "Civil Engineering": ["R", "C"],
  "Electrical Engineering": ["R", "I"],
  "Electronics & Communication": ["R", "I"],
  ...
  "Pharmacy": ["I", "C"],
  "Nursing": ["S", "C"],
  "Allied Health Sciences": ["S", "I"],

  // Business branches
  "Chartered Accountant (CA)": ["C", "I"],
  "MBA / Management": ["E", "S"],
  "Marketing & Sales": ["E", "A"],
  "Finance & Banking": ["C", "I"],
  ...
  // Arts branches
  "Fine Arts & Design": ["A", "S"],
  "Literature & Writing": ["A", "I"],
  "Journalism & Mass Communication": ["A", "S"],
  "Film & Media": ["A", "E"],
  ...
  "Criminal Law": ["E", "S"],
  "Civil Law": ["C", "S"],
  "Constitutional Law": ["E", "C"],
  "Intellectual Property Law": ["A", "C"],

  // Education branches
  "School Teaching": ["S", "C"],
  "College Professor": ["S", "I"],
  "Special Education": ["S", "A"],
  "EdTech & Online Education": ["S", "I"],
  "Coaching & Mentoring": ["S", "E"],

  // Government branches
  "IAS / IPS (UPSC)": ["E", "C"],
  "State Civil Services": ["C", "S"],
  "Public Sector Undertakings": ["C", "E"],
  "Policy & Think Tanks": ["I", "S"],
  "Government Banking (SBI/RBI)": ["C", "E"],

  // Defence branches
  "Indian Army": ["R", "E"],
  "Indian Navy": ["R", "I"],
  "Indian Air Force": ["R", "I"],
  "NDA / CDS": ["R", "E"],
  "Intelligence & Security": ["I", "E"],

  // Technology branches
  "Software Development": ["I", "C"],
  "Data Science & AI/ML": ["I", "C"],
  "Cybersecurity": ["I", "R"],
  "Cloud & DevOps": ["I", "C"],
  "Product Management": ["E", "S"],

  // Science branches
  "Physics Research": ["I", "R"],
  "Chemistry & Materials": ["I", "R"],
  "Biology & Biotechnology": ["I", "S"],
  "Environmental Science": ["I", "S"],
  "Space & Astronomy (ISRO)": ["I", "R"]
};
```

Now let me update the schema to support Holland Codes:

```typescript
// Update to schema.prisma - Add new QuestionType enum values

// Original QuestionType enum (keep these):
// enum QuestionType {
//   INTEREST
//   APTITUDE
//   PERSONALITY
...
// New Holland Code types (add these to QuestionType):
// For the existing schema, we'll keep the existing QuestionType and add a new field:
// hollandCode QuestionType? in the Question model

// Actually, let's add a new HollandCode type to the schema:
// Update the QuestionType enum in schema.prisma to include:
// enum QuestionType {
//   INTEREST
//   APTITUDE
//   PERSONALITY
...
Now let me create the API route for fetching Holland Code questions:

```typescript
// src/app/api/student/test/interest-questions/route.ts
import { NextResponse } from "next/server";
import prisma from "@/lib/prisma";
import { auth } from "@/lib/auth";
import { getQuestionsForClassLevel, HOLLAND_CODE_QUESTIONS } from "@/holland-code-questions";
...
export async function GET() {
  ...
  try {
    ...
    // Check for existing in-progress Holland Code assessment
    ...
    const questions = getQuestionsForClassLevel(classLevel);

    // Randomize and limit questions (similar to existing logic)
    const TOTAL_QUESTIONS = 30; // Adjust based on class level
    ...
    console.error("Interest questions error:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
```

Now let me create the answer calculation logic that maps to Holland Codes and then to career clusters:

```typescript
// src/lib/calculate-holland-codes.ts
import { HollandQuestion } from "@/holland-code-questions";
...
export function calculateHollandScores(
  ...
  const scores: HollandScore = { R: 0, I: 0, A: 0, S: 0, E: 0, C: 0 };
...
export function getDominantCodes(scores: HollandScore, count: number = 3): string[] {
...
export function getCareerRecommendations(dominantCodes: string[]): string[] {
  // This would be implemented based on your existing career clusters
  // For now, it's a placeholder
  return [];
}
```

Let me also update the assessment result calculation to include Holland Code scores:

```typescript
// src/app/api/student/test/submit/route.ts - Updated submit endpoint
import { NextResponse } from "next/server";
import prisma from "@/lib/prisma";
import { auth } from "@/lib/auth";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // For Holland Code assessment, calculate scores by type
    if (assessment.type === "HOLLAND_CODE") {
      ...
      allAnswers.forEach((answer) => {
        const questionType = answer.question.type as "R" | "I" | "A" | "S" | "E" | "C" | "INTEREST";

        // Map INTEREST type to Holland codes based on question text or metadata
        // This would need to be stored in the database or derived from the question

        // For now, assume the question.type is stored as Holland codes
        if (questionType in hollandScores) {
      ...
      await prisma.$transaction(async (tx) => {
        // Clear old results
        await tx.assessmentResult.deleteMany({ where: { assessmentId } });

        // Calculate percentages and save
        const totalAnswers = allAnswers.length;
        const maxScore = totalAnswers * 3; // Approximate max score
        const hollandCodes = Object.entries(hollandScores);

        for (const [code, score] of hollandCodes) {
          // Find or create Holland Code cluster mapping
          // This would need to be implemented based on your existing clusters
        }
      });
    } else {
      ...
      for (const cluster of allClusters) {
        ...
        clusterScores[cluster.id].max = allAnswers.length * 3;
      }
...
Now let me create a comprehensive migration script to add these questions to the database:

```typescript
// prisma/holland-code-seed.ts
import { PrismaClient } from "@prisma/client";
import { HOLLAND_CODE_QUESTIONS } from "./holland-code-questions";

const prisma = new PrismaClient();

async function seedHollandCodeQuestions() {
  console.log("Seeding Holland Code questions...");
  ...
  for (const q of HOLLAND_CODE_QUESTIONS) {
    ...
    const existing = await prisma.question.findFirst({
      where: {
        text: q.text,
        type: "INTEREST" as any, // Keep as INTEREST type for now
        classMin: q.classMin,
        classMax: q.classMax,
      },
    });
        ...
        options: [
          {
            ...
            weights: {
              create: [
                { cluster: { connect: { name: "Engineering" } }, weight: q.type === "R" ? 3 : q.type === "I" ? 2 : 0 },
                { cluster: { connect: { name: "Medical & Healthcare" } }, weight: q.type === "S" ? 3 : q.type === "I" ? 2 : 0 },
                { cluster: { connect: { name: "Business & Commerce" } }, weight: q.type === "E" ? 3 : q.type === "C" ? 2 : 0 },
                { cluster: { connect: { name: "Arts & Humanities" } }, weight: q.type === "A" ? 3 : q.type === "S" ? 1 : 0 },
                { cluster: { connect: { name: "Law & Legal" } }, weight: q.type === "E" ? 2 : q.type === "C" ? 2 : 0 },
                { cluster: { connect: { name: "Education & Teaching" } }, weight: q.type === "S" ? 3 : q.type === "E" ? 1 : 0 },
                { cluster: { connect: { name: "Government & Civil Services" } }, weight: q.type === "C" ? 2 : q.type === "E" ? 1 : 0 },
                { cluster: { connect: { name: "Defence & Security" } }, weight: q.type === "R" ? 2 : q.type === "E" ? 1 : 0 },
                { cluster: { connect: { name: "Technology & IT" } }, weight: q.type === "I" ? 3 : q.type === "C" ? 2 : 0 },
                { cluster: { connect: { name: "Science & Research" } }, weight: q.type === "I" ? 3 : q.type === "R" ? 1 : 0 },
              ],
            },
          },
          {
            ...
            weights: {
              create: [
                { cluster: { connect: { name: "Engineering" } }, weight: q.type === "R" ? 2 : q.type === "I" ? 1 : 0 },
                { cluster: { connect: { name: "Medical & Healthcare" } }, weight: q.type === "S" ? 2 : q.type === "I" ? 1 : 0 },
                { cluster: { connect: { name: "Business & Commerce" } }, weight: q.type === "E" ? 2 : q.type === "C" ? 1 : 0 },
                { cluster: { connect: { name: "Arts & Humanities" } }, weight: q.type === "A" ? 2 : q.type === "S" ? 1 : 0 },
                { cluster: { connect: { name: "Law & Legal" } }, weight: q.type === "E" ? 1 : q.type === "C" ? 1 : 0 },
                { cluster: { connect: { name: "Education & Teaching" } }, weight: q.type === "S" ? 2 : q.type === "E" ? 1 : 0 },
                { cluster: { connect: { name: "Government & Civil Services" } }, weight: q.type === "C" ? 1 : q.type === "E" ? 1 : 0 },
                { cluster: { connect: { name: "Defence & Security" } }, weight: q.type === "R" ? 1 : q.type === "E" ? 1 : 0 },
                { cluster: { connect: { name: "Technology & IT" } }, weight: q.type === "I" ? 2 : q.type === "C" ? 1 : 0 },
                { cluster: { connect: { name: "Science & Research" } }, weight: q.type === "I" ? 2 : q.type === "R" ? 1 : 0 },
              ],
            },
          },
          {
            ...
            weights: {
              create: [
                { cluster: { connect: { name: "Engineering" } }, weight: 0 },
                { cluster: { connect: { name: "Medical & Healthcare" } }, weight: 0 },
                { cluster: { connect: { name: "Business & Commerce" } }, weight: 0 },
                { cluster: { connect: { name: "Arts & Humanities" } }, weight: 0 },
                { cluster: { connect: { name: "Law & Legal" } }, weight: 0 },
                { cluster: { connect: { name: "Education & Teaching" } }, weight: 0 },
                { cluster: { connect: { name: "Government & Civil Services" } }, weight: 0 },
                { cluster: { connect: { name: "Defence & Security" } }, weight: 0 },
                { cluster: { connect: { name: "Technology & IT" } }, weight: 0 },
                { cluster: { connect: { name: "Science & Research" } }, weight: 0 },
              ],
            },
          },
          {
    ...
    created++;
    console.log(`  ✅ Created: "${question.text.substring(0, 40)}..." (ID: ${question.id})`);
  }
...
Now let me create a dedicated module for Holland Code analysis and career recommendations:

```typescript
// src/lib/holland-code-analysis.ts
import { HollandQuestion, HOLLAND_CODE_QUESTIONS } from "@/holland-code-questions";
...
export function calculateHollandProfile(
  questions: HollandQuestion[],
  ...
  questions.forEach((q) => {
    if (answers[q.id.toString()] !== undefined) {
      scores[q.type] += 1;
      answeredCount++;
    }
  });

  const dominantCodes = Object.entries(scores)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3)
    .map(([code]) => code);
...
export function getCareerClustersForHollandCodes(codes: string[]): string[] {
  const clusters: Record<string, number> = {};

  // Mapping of Holland codes to career clusters
  const codeClusterMap: Record<string, string[]> = {
  ...
  codes.forEach((code) => {
    const clusterList = codeClusterMap[code] || [];
...
export function getRecommendations(profile: HollandProfile): string[] {
  const recommendations: string[] = [];

  const { dominantCodes, scores } = profile;

  if (dominantCodes.length > 0) {
    const topCode = dominantCodes[0];

    switch (topCode) {
      case "R":
        recommendations.push(
          "Your strongest interest is in hands-on, practical work. Consider careers in engineering, trades, or technical fields.",
      ...
      case "I":
        recommendations.push(
          "Your strongest interest is in investigative, analytical work. Consider careers in science, research, or technology.",
          "You enjoy solving problems and working with ideas. Look for careers that involve research, analysis, and critical thinking."
        );
        break;
      case "A":
        recommendations.push(
          "Your strongest interest is in artistic, creative work. Consider careers in design, writing, or performing arts.",
          "You prefer unstructured work that allows for self-expression. Look for careers that involve creativity and originality."
        );
        break;
      case "S":
        recommendations.push(
          "Your strongest interest is in social, helping work. Consider careers in education, healthcare, or social services.",
          "You enjoy working with people to inform, train, or help them. Look for careers that involve teaching, counseling, or service."
        );
        break;
      case "E":
        recommendations.push(
          "Your strongest interest is in enterprising, leadership work. Consider careers in business, management, or sales.",
          "You enjoy leading, persuading, and achieving goals. Look for careers that involve sales, marketing, or entrepreneurship."
        );
        break;
      case "C":
        recommendations.push(
          "Your strongest interest is in conventional, organized work. Consider careers in administration, finance, or operations.",
  ...
  if (dominantCodes.length > 1) {
    const secondCode = dominantCodes[1];
    if (secondCode !== topCode) {
      recommendations.push(
        `Your secondary interest in ${secondCode} suggests you might enjoy roles that combine your top two interests.`
      );
    }
  }
...
Now let me update the schema to add a hollandCode field to questions:

```typescript
// In prisma/schema.prisma, update the Question model:
// model Question {
//   ...
//   type        QuestionType
...
Now let me create a summary document for implementing the Holland Code assessment:
 ...
 *
 * This implementation adds career interest assessment based on the Holland Code (RIASEC) model.
 *
 * Key Features:
 * 1. 155 questions across 3 age groups (Class 8-10, 11-12, College+)
 * 2. 6 personality types: Realistic (R), Investigative (I), Artistic (A), Social (S), Enterprising (E), Conventional (C)
 * 3. Automatic career cluster recommendations based on dominant Holland codes
 * 4. Integration with existing assessment system
 *
 * Files Created/Modified:
 * 1. prisma/holland-code-questions/interest-questions.ts - Question data
 * 2. prisma/holland-code-questions/types.ts - Type definitions
 * 3. prisma/holland-code-seed.ts - Database seeding script
 * 4. src/lib/holland-code-analysis.ts - Analysis logic
 * 5. src/app/api/student/test/interest-questions/route.ts - API endpoint
 * 6. src/app/api/student/test/submit/route.ts - Updated submit endpoint
 * 7. prisma/schema.prisma - Updated with hollandCode field
 *
 * Database Setup:
 * 1. Run: npx tsx prisma/holland-code-seed.ts
 * 2. This will add all 155 questions to the database
 *
 * Usage in Student Assessment:
 * - Questions are fetched based on student's class level
 * - Scores are calculated for each Holland code type
 * - Career recommendations are generated based on dominant codes
 *
 * Next Steps:
 * 1. Update the QuestionType enum in schema.prisma to include Holland codes
 * 2. Run the migration script to seed questions
 * 3. Update the student test page to include Holland Code results
 * 4. Create a dedicated results page showing Holland code breakdown
 * 5. Add career recommendations based on Holland codes
 */
```

Would you like me to help you with any specific part of this implementation, such as creating the actual database migration script or updating the student results page to display the Holland code analysis?
